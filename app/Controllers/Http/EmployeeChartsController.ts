...
export default class EmployeeChartsController {
  public async karyawanKehadiran({ response }: HttpContextContract) {
    ...
    const getLastMonths = `
      select distinct substring(time_in::date,0,8) tanggal
      from ${tableSyncPresences}  
      where time_in is not null 
      order by substring(time_in::date,0,8) desc
      limit 3 
    `
    ...
    const selectDaily = `
      select tanggal
          , total
          ,concat(((total / total_employees)*100)::integer,'%') precentage
            ,total_employees
      from (
    ...
    const selectMonthly = `
      select bulan, total
      ,concat(((total / (total_days * total_employees))*100)::integer,'%') precentage
      , total_days, total_employees
      from (
      select substring(time_in::date,0,8) bulan
      , count(p.id) total
      , count(distinct p.time_in::date) total_days
      from presences p
      where p.activity_id = '${activityId}'
        and substring(time_in::date,0,8) between '${startMonth}' and '${endMonth}'
        and EXTRACT(ISODOW FROM time_in::date) < 6
      group by substring(time_in::date,0,8)
      )p
      ,( 
      select count(id) total_employees
      from employees e
