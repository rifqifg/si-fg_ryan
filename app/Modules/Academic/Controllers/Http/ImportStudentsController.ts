  ...
  static async ImportClassification(excelBuffer) {
    ...
    function checkResidence(value) {
      if (value) {
        ...
        const other = "Lainnya";
        return String(value).toLowerCase() == parent.toLowerCase()
          ? "with parent"
          : String(value).toLowerCase() == dormitori.toLowerCase()
          ? "dormitory"
          : String(value).toLowerCase() == boarding.toLowerCase()
          ? "boarding school"
          : String(value).toLowerCase() == other.toLowerCase()
          ? "others"
          : null;
      } else {
        return null;
    ...
    function checkProgram(value) {
      if (value) {
        ...
        return String(value.toLowerCase()) == MH
          ? MH
          : String(value.toLowerCase()) == BD
          ? BD
          : String(value.toLowerCase()) == FD
          ? FD
          : String(value.toLowerCase()) == WM
          ? WM
          : null;
      } else {
        return null;
    ...
    function checkParentEducation(value) {
      if (value) {
        ...
        const D3 = "D3";
        const D4 = "D4";
        return String(value).toLowerCase() == ELEMENTARY_SCHOOL.toLowerCase()
          ? ELEMENTARY_SCHOOL
          : String(value).toLowerCase() == JUNIOR_HIGH_SCHOOL.toLowerCase()
          ? JUNIOR_HIGH_SCHOOL
          : String(value).toLowerCase() == HIGH_SCHOOL.toLowerCase()
          ? HIGH_SCHOOL
          : String(value).toLowerCase() == S1.toLowerCase()
          ? S1
          : String(value).toLowerCase() == S2.toLowerCase()
          ? S2
          : String(value).toLowerCase() == D1.toLowerCase()
          ? D1
          : String(value).toLowerCase() == D2.toLowerCase()
          ? D2
          : String(value).toLowerCase() == D3.toLowerCase()
          ? D3
          : String(value).toLowerCase() == D4.toLowerCase()
          ? D4
          : null;
      } else {
        return null;
    ...
    jsonData.map((value: PayloadImportStudent, index) => {
      if (index > 0) {
        ...
        studentFathers.manyStudentParents.push({
          ...
          min_salary:
            value["Min Salary Ayah"] === undefined
              ? "0"
              : String(value["Min Salary Ayah"]),
          max_salary:
            value["Max Salary Ayah"] === undefined
              ? "0"
              : String(value["Max Salary Ayah"]),
          nik:
            value["NIK Ayah"] === undefined
              ? "0000000000000000"
              : String(value["NIK Ayah"]),
        });
        ...
        studentMothers.manyStudentParents.push({
          ...
          min_salary:
            value["Min Salary Ibu"] === undefined
              ? "0"
              : String(value["Min Salary Ibu"]),
          max_salary:
            value["Max Salary Ibu"] === undefined
              ? "0"
              : String(value["Max Salary Ibu"]),
          nik:
            value["NIK Ibu"] === undefined
              ? "0000000000000000"
              : String(value["NIK Ayah"]),
        });
        ...
        studentGuardians.manyStudentParents.push({
          ...
          min_salary:
            value["Min Salary Wali"] === undefined
              ? "0"
              : String(value["Min Salary Wali"]),
          max_salary:
            value["Max Salary Wali"] === undefined
              ? "0"
              : String(value["Max Salary Wali"]),
          nik:
            value["NIK Wali"] === undefined
              ? "0000000000000000"
              : String(value["NIK Ayah"]),
        });
      }
    ...
    const studentValidator = new CreateManyStudentValidator(students);
    const resultStudentValidator = await validator.validate(studentValidator);
    const dataStudents = await Student.createMany(
      resultStudentValidator.manyStudents
    );

    dataStudents.map((value) => {
      studentFathers.manyStudentParents.map((sf, index) => {
        if (value.$attributes.name == sf.studentId) {
          studentFathers.manyStudentParents[index]["studentId"] =
            value.$attributes.id;
        }
      });
      studentMothers.manyStudentParents.map((sf, index) => {
        if (value.$attributes.name == sf.studentId) {
          studentMothers.manyStudentParents[index]["studentId"] =
            value.$attributes.id;
        }
      });
      studentGuardians.manyStudentParents.map((sf, index) => {
        if (value.$attributes.name == sf.studentId) {
          studentGuardians.manyStudentParents[index]["studentId"] =
            value.$attributes.id;
        }
      });
    });

    const studentFathersValidator = new CreateManyStudentParentValidator(
      studentFathers
    );
    const resultStudentFathersValidator = await validator.validate(
      studentFathersValidator
    );
    await StudentParent.createMany(
      resultStudentFathersValidator.manyStudentParents
    );

    const studentMothersValidator = new CreateManyStudentParentValidator(
      studentMothers
    );
    const resultStudentMothersValidator = await validator.validate(
      studentMothersValidator
    );
    await StudentParent.createMany(
      resultStudentMothersValidator.manyStudentParents
    );

    const studentGuardiansValidator = new CreateManyStudentParentValidator(
      studentGuardians
    );
    const resultStudentGuardianValidator = await validator.validate(
      studentGuardiansValidator
    );
    await StudentParent.createMany(
      resultStudentGuardianValidator.manyStudentParents
    );
  }
}
