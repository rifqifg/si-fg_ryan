...
export default class RencanaPengambilanNilaisController {
  public async index({request, response}: HttpContextContract) {
    ...
    try {
      let data = {}
      if (mode === "page") {
        data = await RencanaPengambilanNilai.query().select('*').paginate(page, limit)
      } else if (mode === "list") {
  ...
  public async store({request, response}: HttpContextContract) {
    const payload = await request.validate({schema: schema.create({
      programSemesterDetailId: schema.string([rules.uuid({version: 4}), rules.trim()]),
      metodePengambilanNilaiId: schema.string([rules.uuid({version: 4}), rules.trim()]),
  ...
  public async show({response, params}: HttpContextContract) {
    ...
    if (!uuidValidation(id)) {
      return response.badRequest({ message: "Rencana Pengambilan Nilain ID tidak valid" });
    }

    try {
      
      const data = await RencanaPengambilanNilai.query().where('id', id).preload('metodePengambilanNilai', mtn => mtn.select('*')).preload('programSemesterDetail', prosemDetail => prosemDetail.select('*')).firstOrFail()
  ...
  public async update({request, response, params}: HttpContextContract) {
    ...
    const payload = await request.validate({schema: schema.create({
      programSemesterDetailId: schema.string.optional([rules.uuid({version: 4}), rules.trim()]),
      metodePengambilanNilaiId: schema.string.optional([rules.uuid({version: 4}), rules.trim()]),
