...
export default class StudentRaport extends BaseModel {
  ...
  public static async insertStudentRaportDetail(studentRaport: StudentRaport) {
    ...
    try {
     ...
     const bukuNilai = await BukuNilai.query().select('*').andWhere(q => (q.where('classId', kelas.id), q.whereBetween('tanggalPengambilanNilai', [id && !hitungUlang ? raport.fromDate : request.body().fromDate!, id && !hitungUlang ? raport.toDate : request.body().toDate!]), q.where('studentId', studentRaport.studentId)))
      ...
      return response.badRequest({message: 'Gagal membuat raport', error: error.message || error})
    }
}
